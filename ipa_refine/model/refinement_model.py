...
from ipa_refine.model.heads import RefinementAuxiliaryHeads
from ipa_refine.model.primitives import Linear, ipa_point_weights_init_
from ipa_refine.np.residue_constants import (
    restype_rigid_group_default_frame,
    restype_atom14_to_rigid_group,
    restype_atom14_mask,
    restype_atom14_rigid_group_positions,
)
from ipa_refine.utils.affine_utils import T, quat_to_rot
from ipa_refine.utils.feats import (
...
class RefinementModel(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.aux_heads = RefinementAuxiliaryHeads(
            config["model"]["heads"],
        )
    ...
    def forward(
        ...
        s = sample['x_features']['node']   #.to(device)
        z = sample['x_features']['edge']   #.to(device)
        f = sample['x_features']['aatype'].long()   #.to(device)
        ...
        # [*, N]
        t = T(sample['x_features']['frames_R'], sample['x_features']['frames_t'])
        ...
        out = {}
        out["sm"] = outputs
        out["sm"]["frames"] = out["sm"]["frames"]
        # out["final_atom_positions"] = atom14_to_atom37(
        #     out["sm"]["positions"][-1], sample['x_features']
        # )
        # out["final_atom_mask"] = sample['x_features']["atom37_atom_exists"]
        # out["final_affine_tensor"] = out["sm"]["frames"][-1]

        out.update(self.aux_heads(out))
