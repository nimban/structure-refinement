...
def get_SepEnc(seq):
    seq_len = len(seq)
    sep = np.abs(np.linspace(0, seq_len, num=seq_len, endpoint=False)[:, None] - np.linspace(0, seq_len, num=seq_len,
                                                                                             endpoint=False)[None, :])
...
def calc_angle_map(data, nan_fill=-4):
    ...
    Calc the ange for a map with three points.
        preprocessing: shape: [N, N, 3, 3].
    """
    ba, bc = (data[:, :, 0, :] - data[:, :, 1, :]), (data[:, :, 2, :] - data[:, :, 1, :])
    angle_radian = np.arccos(
        np.einsum('ijk,ijk->ij', ba, bc) / (np.linalg.norm(ba, axis=-1) * np.linalg.norm(bc, axis=-1)))
...
def calc_geometry_maps(structure_file, res_range=None,
                       geometry_types=['Ca1-Ca2', 'Cb1-Cb2', 'N1-O2', 'Ca1-Cb1-Cb2', 'N1-Ca1-Cb1-Cb2',
                                       'Ca1-Cb1-Cb2-Ca2'],
    ...
    geometry_types = set(geometry_types) & set(
...
with open(__MYPATH__ + "/heavy_atoms.pkl", "rb") as f:
    heavy_atoms = pickle.load(f)
...
def get_atom_emb(pdb_file, res_range, model_id=0, chain_id=0):
    ...
    '''
    res_num = res_range[1] - res_range[0] + 1
    frame_atoms = np.zeros((res_num, 3, 3))  # N, CA, C
    res_names = np.zeros((res_num))
    ...
    for residue in residue_list:
        ...
        #
        res_names[residue.id[1] - res_range[0]] = random.randrange(20)
        # res_names[residue.id[1]-res_range[0]] = resNameToId(residue['resname'])  TODO : convert residue name to index
        ...
        frame_atoms[residue.id[1] - res_range[0]] = np.stack(
    ...
    for i, _item in enumerate(atom_embs):
        if not np.isscalar(_item):  # not -1, no preprocessing
            atom_nums[i] = _item.shape[0]
            embedding[i, :_item.shape[0], :] = _item

    return embedding, frame_atoms, res_names


# Construct frame per residue
def rigidFrom3Points(x):  # 3 coordinate points
    x1, x2, x3 = x  # N, CA, C
    v1 = x3 - x2
    v2 = x1 - x2
    e1 = v1 / np.linalg.norm(v1)
    u2 = v2 - np.dot(e1, np.dot(np.transpose(e1), v2))
