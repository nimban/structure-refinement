...
from ipa_refine.np import residue_constants
from ipa_refine.utils import feats
from ipa_refine.utils.affine_utils import T
...
def drmsd_loss(out, batch, config):
    gt_ca_atoms = batch["all_atom_positions"][..., :, 1, :]
    pred_ca_atoms = out['sm']["positions"][-1][..., :, 1, :]
    ...
    loss = torch.tensor(drmsd.mean())
    return loss


def eval_drmsd_loss_batch(x, y):
    gt_ca_atoms = x
    pred_ca_atoms = y
    drmsd = np.zeros(len(gt_ca_atoms))
    for i in range(len(gt_ca_atoms)):
...
class RefinementModelLoss(nn.Module):
    """Aggregation of the various losses described in the supplement"""

    def __init__(self, config):
        super(RefinementModelLoss, self).__init__()
        self.config = config

    def forward(self, out, batch):
        batch = batch['y_features']
        ...
        loss_fns = {
            ...
            # ),
            "drmsd": lambda: drmsd_loss(        # calculated only for reporting
                out,
                batch,
                self.config.drmsd,
            )
