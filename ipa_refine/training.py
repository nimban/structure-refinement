'''
Run WandB experiment for training IPA model for refinement
...
# Configs will be overwritten if initialized through sweep command
...
class RefinementModelWrapper(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        return {"loss": loss, "drmsd": loss_metrics["drmsd"]}

    def validation_step(self, batch, batch_idx):
        # Calculate validation loss
        outputs = self(batch)
        loss, loss_metrics = self.loss(outputs, batch)
        return {"val_loss": loss, "val_drmsd": loss_metrics["drmsd"]}

    def training_epoch_end(self, outputs):
        avg_loss = torch.stack([x['loss'] for x in outputs]).mean()
        avg_drmsd = torch.stack([x['drmsd'] for x in outputs]).mean()
        self.log('loss', avg_loss)
        wandb.log({"train_loss": avg_loss, "train_drmsd": avg_drmsd})
        return None

    def validation_epoch_end(self, outputs):
        avg_loss = torch.stack([x['val_loss'] for x in outputs]).mean()
        avg_drmsd = torch.stack([x['val_drmsd'] for x in outputs]).mean()
        self.log('val_loss', avg_loss)
        wandb.log({"val_loss": avg_loss, "val_drmsd": avg_drmsd})
        torch.save(self.model.state_dict(), os.path.join(wandb_run.dir, 'model_weights'))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_size", type=int, default=4000,
        help="Random seed"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Data List with examples to train/test"
    )
    parser.add_argument(
