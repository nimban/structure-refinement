...
class EdgeApplyModule(nn.Module):
    ...
    def forward(self, edges):
        x = torch.cat((edges.src['nfeat'], edges.data['efeat'], edges.dst['nfeat']), 1)
...
def reduce_func(node):
    ...
    feat = torch.cat((node.data['nfeat'], attn_feat), 1)
    return {'_nfeat': feat}
...
class GNN(nn.Module):
    ...
    def forward(self, G):
        for layer in self.layers: G = layer(G)
